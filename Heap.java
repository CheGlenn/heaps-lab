...
public class Heap<T> implements LinearStructure<T> {
  ...
  /**
   * Helper for heapDown, returns index of smaller child of i
   * @param i
   */
  int heapDownHelper(int i){
    T leftChild = this.values.get(this.left(i));
    T rightChild = this.values.get(this.right(i));

      if (priority.compare(leftChild, rightChild) < 0) {
        return this.left(i);
      } 
      else {
        return this.right(i);
  ...
  void heapDown(int i) {
    ...
    //left and right child of current index
    T leftChild = this.values.get(this.left(i));
    T rightChild = this.values.get(this.right(i));

    /*check if the left child and right child are both null
     * if both are null return
     * if left child is not null, and current value is greater than left child, swap places
     * if right child is not null, and current value is greater than right child, swap places
     * 
    */
    if (leftChild == null && rightChild == null) {
      ...
      if (priority.compare(this.values.get(i), leftChild) > 0) {
        ArrayUtils.swap(values, i, this.left(i));
        heapDown(this.left(i));
      }
    } else if (rightChild != null) {
      if (priority.compare(this.values.get(i), rightChild) > 0) {
        ArrayUtils.swap(values, i, this.right(i));
        heapDown(this.right(i));
      }
    }

    //compare current element to its parent, if current is larger than parent swap values
   ...
   */
  void heapUp(int i) {
    //define parent for pos i
    T parent = this.values.get(this.parent(i));

    if (i <= 0) {
      return;
    } else {
      //compare current element to its parent, if current is larger than parent swap values
      if (priority.compare(this.values.get(i), parent) < 0) {
        ArrayUtils.swap(values, i, this.parent(i));
        heapUp(this.parent(i));
      }
